...
for i in range(int(pocet_bytu)):
    ...
    spotreba_studene_vody = st.number_input(f"Spotřeba studené vody bytu {i+1} (m³):", min_value=0.0, key=f"spotreba_studene_vody_{i}")
    spotreba_teple_vody = st.number_input(f"Spotřeba teplé vody bytu {i+1} (m³):", min_value=0.0, key=f"spotreba_teple_vody_{i}")
